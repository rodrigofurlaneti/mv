...
export class ProductProvider {
  ...
  public max_page_partner_size: number = 50;
  ...
  public listProdutoPorCategoria(categoria: number, page: number = 1): Observable<ProductLoja[]> {
    return this.defaultHttp.cached_request(`api/produto/produtosPorCategoria/${categoria}/${this.currentStorage}?inicio=${page}&quantidade=${this.max_page_size}`)
      .pipe(map(data => ProductLoja.listFromData(data)));
  }

  public listOfertasPorCategoria(categoria: number, page: number = 1): Observable<ProductLoja[]> {
    return this.defaultHttp.request("get", `api/produto/produtosPorCategoria/${categoria}?inicio=${page}&quantidade=${this.max_page_partner_size}`)
      .pipe(map(data => ProductLoja.listFromData(data)));
  }
  ...
  deleteCategoria(categoria: any) {
    return this.defaultHttp.request('delete', 'api/CategoriaProduto/' + categoria);
  }

  deleteDepartamento(departamento: any) {
    return this.defaultHttp.request('delete', 'api/DepartamentoProduto/' + departamento);
  }
}
