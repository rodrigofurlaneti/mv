...
@IonicPage()
@Component({
  selector: 'page-client-register-simple',
  templateUrl: 'client-register-simple.html'
})
export class ClientRegisterSimplePage {
  ...
  private procedLogin(): any {
    ...
    if (this.isFacebookRegister) {
      service = this.authProvider.authenticate({ name: this.clientForm.value.cpf.replace(".","").replace("-","").replace(".",""), password: this.facebookId });
    } else {
      service = this.authProvider.authenticate({ name: this.clientForm.value.cpf.replace(".","").replace("-","").replace(".",""), password: this.clientForm.value.senha });
    }
